...
#include "../../Downloads/lab/curl.h"

#ifndef NLOHMANN_JSON_VERSION_MAJOR
#include "../../Downloads/lab/json.hpp"
#endif
...
namespace templet {
	...
	bool everest_engine::submit(everest_task&t) {
		...
		if (_code == 200 || _code == 201) {
			...
			ev._id = j["id"].get<std::string>();
			ev._state = j["state"].get<std::string>();
			ev._task = &t;
	
			_submitted.push_back(ev);

			t._job_ID = j["id"].get<std::string>();
			t._idle = false;
			t._done = false;
