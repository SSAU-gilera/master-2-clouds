...
#include "../../Downloads/lab/basesim.hpp"
#include <iostream>
...
struct Link :public templet::actor {
	...
	static void on_t_adapter(templet::actor* a, templet::task* t) {
		((Link*)a)->on_t(*(basesim_task*)t);
	}

	Link(templet::engine& e, templet::basesim_engine& te_base) :Link() {
	...
	void engines(templet::engine& e, templet::basesim_engine& te_base) {
	...
	inline void on_t(basesim_task& t) {
		/*$TET$mediator$t*/
		t.delay(1);
		for (int k = 0; k < N; k++) C[i][inner.j] += A[i][k] * B[k][inner.j];
		/*$TET$*/
	}
